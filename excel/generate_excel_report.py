...
def build_workbook():
    ...
    r += 1
    detail_line(r, "見る場所", "「希望休申請一覧」シートの判定列(G列)・必要人数列(K列)・交代要員の提案列(L列)")
    r += 1
    detail_line(r, "判断の目安", "早番Aは交代不要。早番B・夜勤で交代要員が提案されていれば、その人に打診したうえで承認する")
    r += 1
    note_cell2 = detail_line(
        r, "どうしても休む場合",
        "交代要員の「候補提案」まではシステムが自動で行うが、実際にシフト表へ反映し本人へ"
        "連絡するのは管理者の役割。L列に候補が出ない場合はSTEP4の対応案も検討する",
    )
