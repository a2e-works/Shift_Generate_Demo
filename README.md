...
データはブラウザのメモリ上(このタブを開いている間だけ)に保持され、他の人とは共有されません。
タブを閉じる・再読み込みすると入力内容は消去されます。「サーバー無しでも申請〜承認のやり取りが
その場で動く」ことを、実データを残さずに1人で試すためのデモです。
...
`dashboard/index.html` はブラウザのメモリ上だけでデータを保持する単体HTMLファイルとして動作し、
...
- `dashboard/index.html` はブラウザのメモリ上だけで動くプロトタイプであり、データは保存されない
  (タブを閉じる・再読み込みすると消去される)。複数人・複数端末でのリアルタイム共有、
  認証、通知機能などは持たない。
